...
create_fp_node <- function(item, count = 1, parent = NULL) {
...
build_fp_tree <- function(transactions, min_support, verbose = TRUE) {
  ...
  if (length(frequent_items) == 0) {
    fp_tree <- new.env(parent = emptyenv())
    fp_tree$root <- create_fp_node("root", 0, NULL)
    fp_tree$header_table <- list()
    fp_tree$item_counts <- item_counts
    return(fp_tree)
  }
  ...
  fp_tree$root <- create_fp_node("root", 0, NULL)
  fp_tree$header_table <- list()
  fp_tree$item_counts <- item_counts[frequent_items]
  ...
  for (transaction in transactions) {
    ...
    filtered_items <- frequent_items[frequent_items %in% transaction]
...
insert_transaction_fp <- function(fp_tree, items, count = 1) {
  ...
  for (item in items) {
    ...
    if (item %in% names(current_node$children)) {
      ...
      # Create new node
      new_node <- create_fp_node(item, count, current_node)
...
mine_fp_tree <- function(fp_tree, min_support, n_transactions, verbose = TRUE) {
  ...
  for (i in length(items):1) {
    ...
    item_count <- fp_tree$header_table[[item]]$count
    support <- item_count / n_transactions
    ...
    if (length(conditional_patterns) > 0) {
      ...
      if (length(conditional_tree$header_table) > 0) {
        ...
        for (pattern_name in names(conditional_patterns_result)) {
          ...
          new_items <- c(pattern$pattern, item)
          new_pattern_name <- paste(new_items, collapse = ",")

          frequent_patterns[[new_pattern_name]] <- list(
            pattern = new_items,
...
build_conditional_fp_tree <- function(conditional_patterns, min_support, n_transactions) {
  if (length(conditional_patterns) == 0) {
    ...
    tree$root <- create_fp_node("root", 0, NULL)
    tree$header_table <- list()
    tree$item_counts <- numeric(0)
    return(tree)
  }

  # Count item frequencies in conditional patterns
  item_counts_map <- new.env(parent = emptyenv())

  for (p in conditional_patterns) {
    for (itm in p$pattern) {
      if (exists(itm, envir = item_counts_map)) {
        item_counts_map[[itm]] <- item_counts_map[[itm]] + p$count
      } else {
        item_counts_map[[itm]] <- p$count
      }
    }
  }

  item_counts <- unlist(as.list(item_counts_map))

  # Use the same support threshold as the main FP-tree
  min_count <- ceiling(min_support * n_transactions)
  ...
  # Build conditional FP-tree
  conditional_tree <- new.env(parent = emptyenv())
  conditional_tree$root <- create_fp_node("root", 0, NULL)
  conditional_tree$header_table <- list()
  conditional_tree$item_counts <- item_counts[frequent_items]
  ...
  for (pattern_info in conditional_patterns) {
    filtered_items <- frequent_items[frequent_items %in% pattern_info$pattern]
    if (length(filtered_items) > 0) {
      insert_transaction_fp(conditional_tree, filtered_items, pattern_info$count)
    }
  }
...
generate_rules_from_patterns <- function(frequent_patterns, trans_matrix, min_confidence, min_lift) {
  ...
  for (pattern_name in names(frequent_patterns)) {
    ...
    if (!is.null(pattern_info$pattern) && is.character(pattern_info$pattern)) {
      items <- pattern_info$pattern
    } else {
      items <- pattern_name
    }

    # Handle both vector of items and comma-separated string
    if (length(items) == 1 && grepl(",", items)) {
      itemset <- strsplit(items, ",")[[1]]
    } else {
      itemset <- items
    }
...
head.association_rules <- function(x, n = 6, ...) {
  ...
  for (i in 1:nrow(top_rules)) {
    cat(sprintf("%d. %s => %s\n", i, top_rules$antecedent[i], top_rules$consequent[i]))
    cat(sprintf(
      "   Sup: %.3f | Conf: %.3f | Lift: %.3f | Conv: %.3f\n\n",
